...
{
    public abstract class BaseApiAsyncController<T> : ControllerBase where T : CiuchAppBaseModel
    {
        protected readonly ICrudService<T> _service;
        protected readonly ILogger<T> _logger;

        public BaseApiAsyncController(ICrudService<T> service, ILogger<T> logger)
        {
            _service = service;
            _logger = logger;
        }
